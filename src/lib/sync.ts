...
export async function syncAll(db: Database.Database): Promise<SyncResult> {
  ...
  if (!fs.existsSync(PROJECTS_DIR)) {
    return { projectsScanned: 0, sessionsIndexed: 0, durationMs: 0 };
  }
  ...
  for (const folder of projectFolders) {
    ...
    // Try sessions-index.json for authoritative path
    const indexPath = path.join(projectDir, "sessions-index.json");
    let authoritativePath: string | null = null;
    const indexEntries = new Map<
      string,
      { firstPrompt?: string; summary?: string; created?: string; modified?: string; gitBranch?: string; isSidechain?: boolean }
    >();

    if (fs.existsSync(indexPath)) {
      try {
        const index = JSON.parse(fs.readFileSync(indexPath, "utf-8"));
        authoritativePath = index.originalPath || null;
    ...
    // Extract cwd from JSONL if no authoritative path
    if (!authoritativePath && jsonlFiles.length > 0) {
      for (const jf of jsonlFiles) {
        if (authoritativePath) break;
        try {
          const firstLines = fs
            .readFileSync(path.join(projectDir, jf), "utf-8")
            .split("\n")
            .slice(0, 30);
    ...
    const originalPath = authoritativePath || folder;
    const projectName = authoritativePath ? path.basename(authoritativePath) : folder;
    ...
    for (const jsonlFile of jsonlFiles) {
      ...
      // Incremental: skip if already indexed at same mtime
      const existing = db
      ...
      await indexSession(db, folder, sessionId, jsonlPath, mtime, indexEntries.get(sessionId));
      sessionsIndexed++;
    }
...
async function indexSession(
  ...
  jsonlPath: string,
  mtime: number,
  indexEntry?: { firstPrompt?: string; summary?: string; created?: string; modified?: string; gitBranch?: string; isSidechain?: boolean }
): Promise<void> {
  ...
  const atomicReindex = db.transaction(() => {
    ...
    // FTS
    db.prepare(`
      INSERT INTO sessions_fts (session_id, first_prompt, summary) VALUES (?, ?, ?)
...
function syncStatsCache(db: Database.Database) {
